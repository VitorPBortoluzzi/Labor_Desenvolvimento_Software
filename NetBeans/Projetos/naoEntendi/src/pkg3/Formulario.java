...
public class Formulario extends javax.swing.JFrame {
    ...
    private void registrarLog(String acao, String detalhe) {
        ...
        txtA_area.append(mensagemLog);

        // Rola o scroll automaticamente para o final do texto
        txtA_area.setCaretPosition(txtA_area.getDocument().getLength());
    }
    ...
    private void btn_salvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_salvarActionPerformed
        ...
        if (chkBox_Astronomia.isSelected()) {
            interesses.add("Astronomia");
                ...
                // VERIFICA SE É UMA ATUALIZAÇÃO (EDIÇÃO) OU UM NOVO REGISTRO
        if (linhaEdicao != -1) {
            // Atualiza a pessoa existente
            Pessoa pEditada = listaPessoas.get(linhaEdicao);
            pEditada.nome = nome;
            pEditada.sexo = sexo;
            pEditada.idioma = idioma;
            pEditada.interesses = interesses;

            carregarTabela();
            registrarLog("EDITADO", "Registro atualizado: " + pEditada);
            linhaEdicao = -1; // Reseta a edição
        } else {
            // Cria um novo registro
            Pessoa pNova = new Pessoa(nome, sexo, idioma, interesses);
            listaPessoas.add(pNova);
            carregarTabela();
            registrarLog("SALVO", "Pessoa cadastrada: " + pNova);
        }
...
//        }
//      
//        registrarLog("SALVO", "Pessoa cadastrada: " + p);
    ...
    private void btn_excluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_excluirActionPerformed
        int linha = jTable_arquivotxt.getSelectedRow();
        if (linha == -1){
        ...
        if (resposta == JOptionPane.YES_OPTION){
            Pessoa pRemovida = listaPessoas.get(linha);
...
//            jTable_arquivotxt.remove(linha);
    ...
    private void btn_editarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_editarActionPerformed
        ...
        txtF_nome.setText(p.nome);
        
        if (p.sexo == 'M'){
            rdoBtn_masculino.setSelected(true);
        } else {
            rdoBtn_feminino.setSelected(true);
        }
        ...
        if(p.interesses != null){
            for (String interesse : p.interesses) {
                if (interesse.equalsIgnoreCase("Tecnologia")) {
                    chkBox_Tecnologia.setSelected(true);
                }
                if (interesse.equalsIgnoreCase("Astronomia")) {
                    chkBox_Astronomia.setSelected(true);
                }
                if (interesse.equalsIgnoreCase("Esportes")) {
                    chkBox_Esportes.setSelected(true);
                }
            }
        }
        
        if (p.idioma != null) {
            cmbBox_idiomas.setSelectedItem(p.idioma);
        }
        
        registrarLog("Editar", "Pessoa Editada: " + p);
